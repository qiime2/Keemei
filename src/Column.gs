function markDuplicates_(sheet, column, state) {
  var duplicates = findDuplicates_(sheet, column);

  for (var i = 0; i < duplicates.length; i++) {
    var duplicate = duplicates[i];
    updateState_(state, duplicate, Status.ERROR, "Duplicate cell");
  }
};
...
function markInvalidCells_(sheet, column, state, regex, invalidCharactersStatus,
